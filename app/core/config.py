...
class Settings(BaseSettings):
    ...
    TWITTER_BEARER_TOKEN: Optional[str] = Field(
        default=None, description="Twitter bearer token")

    # Firebase Configuration
    FIREBASE_PROJECT_ID: str = Field(
        default="afya-a1006", description="Firebase project ID")
    FIREBASE_STORAGE_BUCKET: str = Field(
        default="stem-grad-assistant.appspot.com", description="Firebase storage bucket")
    FIREBASE_CONFIG: Optional[str] = Field(
        default=None, description="Firebase service account JSON")

    # Application Settings
    APP_NAME: str = Field(
        default="STEM Graduate Admissions Assistant", description="Application name")
    ...
    # Scraping Settings
    ENABLE_BACKGROUND_SCRAPING: bool = Field(
        default=False, description="Enable background scraping")
    SCRAPING_INTERVAL_HOURS: int = Field(
        default=24, description="Scraping interval in hours")
    MAX_CONCURRENT_SCRAPES: int = Field(
        default=5, description="Max concurrent scraping jobs")
...
class FirebaseManager:
    ...
    async def get_document(self, collection: str, doc_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Error getting document {doc_id} from {collection}: {e}")
            return None
    ...
    async def update_document(self, collection: str, doc_id: str, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error updating document {doc_id} in {collection}: {e}")
            return False
        ...
        try:
            ...
            if isinstance(search_term, str) and search_field in ['research_areas', 'specializations']:
                query = self.db.collection(collection).where(
                    search_field, 'array_contains', search_term)
            else:
            ...
            logger.error(
                f"Error searching {collection} for {search_term}: {e}")
            return []
